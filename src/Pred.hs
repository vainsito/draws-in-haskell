...
import Dibujo(Dibujo, foldDib, change)
type Pred a = a -> Bool
...
cambiar predicado figura = change (\x -> if predicado x then figura x else figura x)

-- Alguna básica satisface el predicado.
anyDib :: Pred a -> Dibujo a -> Bool
-- Uso foldDib ya que tengo que recorrer todo el dibujo y ver si alguno cumple el predicado
anyDib predicado = foldDib predicado id id id (\_ _ a b -> a || b) (\_ _ a b -> a || b) (||)

-- Todas las básicas satisfacen el predicado.
allDib :: Pred a -> Dibujo a -> Bool
-- Debo recorrer todo el dibujo y ver si todas las figuras cumplen el predicado por eso uso foldDib
allDib predicado = foldDib predicado id id id (\_ _ a b -> a &&b) (\_ _ a b -> a && b) (&&)
